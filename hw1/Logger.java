...
public class Logger {
    ...
    public static void writelog(String nick, String text) {
        ...
        try (FileOutputStream fos = new FileOutputStream("hw1\\" + nick + "_log.txt", true)) {
            byte[] buffer = (text).getBytes();
            fos.write(buffer, 0, buffer.length);
        } catch (IOException ex) {
            File file =new File(".",nick + "_log.txt");
            writelog( nick,  text);
        }
    }
    ...
    public static String readlog(String nick) {
        ...
        try (FileInputStream fin = new FileInputStream("hw1\\" + nick + "_log.txt")) {
            String out = "";
